...
export function updateCoverflow() {
    ...
    items.forEach((item, index) => {
        ...
        let translateX = offset * 220; // Позиція по X
        let translateZ = -absOffset * 200; // Позиція по Z (глибина)
        let rotateY = -sign * Math.min(absOffset * 60, 60); // Кут повороту
        let opacity = 1 - (absOffset * 0.2); // Прозорість
        let scale = 1 - (absOffset * 0.1); // Масштаб
